...
class APIManager: SessionManager {
    ...
    static let consumerKey = "Oyq2RHqAHC7aQjmiffYJwAEms"
    static let consumerSecret = "GWQUDPJqWBYP5v87VX4TWhqjzAXv9vJlem2okLhbqSwqfNqCwv"
    ...
    func logout() {
        ...
        //self.oauthManager.client.credential.oauthToken = ""
        logout()
    ...
    func unfavorite(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        let urlString = "https://api.twitter.com/1.1/unfavorites/create.json"
        let parameters = ["id": tweet.id]
        request(urlString, method: .post, parameters: parameters, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
            if response.result.isSuccess,
                let tweetDictionary = response.result.value as? [String: Any] {
